...
def remove_shots_from_ref(ref_df, number_shots=5):
    ...
    events_to_drop = ref_df.index[
        ref_df['Endtime'] <= ref_df.iloc[ref_n_shot_index]['Endtime']].tolist()
...
def build_matrix_from_selected_rows(data_frame, selected_indexes_list):
    ...
    for n, idx in enumerate(selected_indexes_list):
        matrix_data[0, n] = data_frame.loc[
            idx].Starttime  # start time for event n
...
def compute_tp_fp_fn(pred_events_df, ref_events_df):
    ...
    ref_1st_round = build_matrix_from_selected_rows(ref_events_df,
                                                    ref_pos_indexes)
    ...
    ref_2nd_round = build_matrix_from_selected_rows(ref_events_df,
                                                    ref_unk_indexes)
...
def compute_scores_per_class(counts_per_class):
    ...
    for cl in counts_per_class.keys():
        ...
        precision = tp / (
                    tp + fp) if tp + fp != 0 else MIN_EVAL_VALUE  # case where no predictions were made
        ...
        scores_per_class[cl] = {"precision": precision, "recall": recall,
                                "f-measure": fmeasure}
...
def build_report(main_set_scores, scores_per_miniset, scores_per_audiofile,
                 save_path, main_set_name="EVAL", team_name="test_team",
                 **kwargs):
    ...
    with open(os.path.join(save_path,
                           "Evaluation_report_" + team_name + "_" + main_set_name + '_' + date_string + '.json'),
              'w') as outfile:
...
def evaluate(pred_file_path, ref_file_path, team_name, dataset, savepath,
             metadata=[]):
    ...
    for subset in list_of_subsets:
        gt_file_structure[dataset][subset] = [
            os.path.basename(fl)[0:-4] + '.wav' for fl in
            glob.glob(os.path.join(ref_file_path, subset, "*.csv"))]
    ...
    for audiofilename in list(pred_events_by_audiofile.keys()):
        ...
        ref_events_this_audiofile_all = pd.read_csv(
            os.path.join(ref_file_path, inv_gt_file_structure[audiofilename],
        ...
        counts_per_audiofile[audiofilename] = {"TP": tp_count, "FP": fp_count,
                                               "FN": fn_count,
    ...
    for miniset in dataset_metadata[dataset].keys():
        if metadata:
            for cl in dataset_metadata[dataset][miniset].keys():
                list_all_audiofiles.extend(
                    dataset_metadata[dataset][miniset][cl])
        else:
    ...
    for audiofilename in list_all_audiofiles:
        if audiofilename not in counts_per_audiofile.keys():
            ref_events_this_audiofile = pd.read_csv(os.path.join(ref_file_path,
                                                                 inv_gt_file_structure[
                                                                     audiofilename],
                                                                 audiofilename[
                                                                 0:-4] + '.csv'),
            ...
            total_n_pos_events_in_audiofile = len(
                select_events_with_value(ref_events_this_audiofile,
                                         value=POS_VALUE))
            counts_per_audiofile[audiofilename] = {"TP": 0, "FP": 0,
    ...
    for data_set in list_sets_in_mainset:
        ...
        if metadata:
            list_classes_in_set = list(
                dataset_metadata[dataset][data_set].keys())
            ...
            for cl in list_classes_in_set:
                # print(cl)
                list_audiofiles_this_class = \
                dataset_metadata[dataset][data_set][cl]
                tp = 0
                fn = 0
                fp = 0
                ...
                for audiofile in list_audiofiles_this_class:
                    scores_per_audiofile[
                        audiofile] = compute_scores_from_counts(
                    ...
                    total_n_pos_events_this_class = total_n_pos_events_this_class + \
                                                    counts_per_audiofile[
                                                        audiofile][
                                                        "total_n_pos_events"]
                    total_n_events_set = total_n_events_set + \
                                         counts_per_audiofile[audiofile][
                                             "total_n_pos_events"]

                # counts_per_class[cl] = {"TP":tp, "FN": fn, "FP": fp, "total_n_pos_events_this_class": total_n_pos_events_this_class}
                counts_per_class_per_set[data_set][cl] = {"TP": tp, "FN": fn,
                                                          "FP": fp,
                                                          "total_n_pos_events_this_class": total_n_pos_events_this_class}
                counts_per_set[data_set] = {"TP": tp_set, "FN": fn_set,
                                            "FP": fp_set,
            ...
            for audiofile in list_audiofiles_in_set:
                ...
                total_n_pos_events_this_set = total_n_pos_events_this_set + \
                                              counts_per_audiofile[audiofile][
                                                  "total_n_pos_events"]
                counts_per_set[data_set] = {"TP": tp, "FN": fn, "FP": fp,
                                            "total_n_pos_events_this_set": total_n_pos_events_this_set}
    ...
    overall_scores = {"precision": stats.hmean(
        [scores_per_set[dt]["precision"] for dt in scores_per_set.keys()]),
                      "recall": stats.hmean(
                          [scores_per_set[dt]["recall"] for dt in
                           scores_per_set.keys()]),
                      "fmeasure (percentage)": np.round(stats.hmean(
                          [scores_per_set[dt]["f-measure"] for dt in
                           scores_per_set.keys()]) * 100, 3)
                      }
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ref_files_path', type=str,
                        help='path to the ground truth csvs folder')
    parser.add_argument('-metadata', type=str,
                        help="path for metadata json. Participants may ignore this option.")
    parser.add_argument('-team_name', type=str, help='team identification')
    parser.add_argument('-dataset', type=str,
                        help="which set to evaluate: EVAL or VAL")
    parser.add_argument('-savepath', type=str,
                        help="path where to save the report to")
    args = parser.parse_args()
    # print(args)

    # evaluate( args.pred_file, args.ref_files_path, args.team_name, args.dataset, args.savepath, args.metadata)
    evaluate(args.pred_file, args.ref_files_path, args.team_name, args.dataset,
             args.savepath)
